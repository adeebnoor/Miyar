...
test('blank input still asks for required information rather than fabricating a result',async()=>{
 const a=app('en');try{a.fill('');a.submit();await settle();assert.equal(a.$('form-error').hidden,false);assert.match(a.w.document.querySelector('.analysis-feedback').textContent,/Complete the required input/i);assert.deepEqual(a.errors,[]);}finally{a.dom.window.close();}
});
