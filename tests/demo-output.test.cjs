...
function app(locale='en'){
 ...
 return {w,dom,$,fill,submit,errors};
}
test('short strategic objectives produce a visible guidance result instead of a silent form state',async()=>{
 for(const locale of ['ar','en']){const a=app(locale);try{
  a.fill(locale==='ar'?'التحول الرقمي':'Digital transformation');a.submit();await settle();
  ...
  assert.ok(a.w.document.querySelector('.analysis-feedback'));
  a.fill(locale==='ar'?'مهندس مدني':'Civil Engineer');a.submit();await settle();assert.match(a.$('result-content').textContent,/214201/);
  assert.deepEqual(a.errors,[]);
 }finally{a.dom.window.close();}}
});
test('blank input still asks for required information rather than fabricating a result',async()=>{
 const a=app('en');try{a.fill('');a.submit();await settle();assert.equal(a.$('form-error').hidden,false);assert.doesNotMatch(a.$('form-error').textContent,/214\d{3}/);assert.ok(a.w.document.querySelector('.analysis-feedback'));assert.deepEqual(a.errors,[]);}finally{a.dom.window.close();}
});
